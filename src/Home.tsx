...
export default function Home() {
  ...
  const handleCreateProject = async (title: string) => {
    ...
    const { data, error } = await supabase
      .from('Projects')
      .insert([{ user_id: user.id, projectTitle: title }])
      .select();
      ...
      <div className="flex-1 md:ml-64 flex flex-col min-h-screen w-full max-w-full overflow-x-hidden">
        ...
        {currentView === 'calendar' && (
          <main className="flex-1 p-8 md:p-12 w-full">
            <h1 className="text-2xl font-bold">Full Calendar View Coming Soon</h1>
          </main>
        )}
              ...
              <div className="bg-gray-200 p-1 rounded-full flex items-center justify-between mb-6 overflow-x-auto hide-scrollbar">
                {['All Tasks', 'Pending', 'In-Progress', 'Complete'].map(tab => (
                  <button
                    ...
                    className={`flex-1 min-w-[90px] text-center py-2 text-sm font-medium rounded-full transition-all ${activeTab === tab ? 'bg-white shadow-sm text-black' : 'text-gray-500 hover:text-gray-800'
                      }`}
                  >
                    {tab}
                  </button>
