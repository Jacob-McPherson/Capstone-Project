...
export default function CalendarTaskModal({ isOpen, onClose, onAddTask, activeProject, projects }: CalendarTaskModalProps) {
  ...
  // When the modal opens, default the dropdown to whatever project folder they are currently looking at
  useEffect(() => {
    if (isOpen) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalDueDate = `${date}T${timeString}:00`;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-3 bg-white/5 px-4 py-3.5 rounded-xl border border-white/5 focus-within:border-white/20 transition-colors">
              ...
              <select 
                ...
                className="bg-transparent text-white outline-none w-full cursor-pointer appearance-none text-sm font-medium"
              >
            ...
            <div className="flex items-center gap-3 bg-white/5 px-4 py-3.5 rounded-xl border border-white/5 focus-within:border-white/20 transition-colors">
              ...
              <input 
                ...
                className="bg-transparent text-white outline-none w-full cursor-pointer color-scheme-dark text-sm font-medium" 
                required 
              />
            </div>
            ...
            <div className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border transition-colors ${date ? 'bg-white/5 border-white/5 focus-within:border-white/20' : 'bg-white/5 border-transparent opacity-40'}`}>
              ...
              <input 
                ...
                disabled={!date}
                className="bg-transparent text-white outline-none w-full cursor-pointer color-scheme-dark text-sm font-medium disabled:cursor-not-allowed" 
              />
            </div>
