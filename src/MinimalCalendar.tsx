...
export default function MinimalCalendar({ quests }: CalendarProps) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const questsDueToday = quests.filter(q => {
      if (!q.dueDate || q.status === 'Complete') return false;
      const taskDateOnly = q.dueDate.split('T')[0];
      return taskDateOnly === dateString;
    }).length;
  ...
  return (
    <div className="p-6 bg-white rounded-xl border border-gray-200 shadow-sm w-full">
      ...
      <div className="grid grid-cols-7 gap-2 mb-2">
        {dayNames.map(day => (
          <div key={day} className="aspect-square flex items-center justify-center text-xs font-bold text-gray-500">
            {day}
          </div>
        ))}
      </div>
